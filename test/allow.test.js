...
lab.test('happy', fin => {
  const permspecs = {
    ...
    bbb: {
      perms: [{ p: { usr$: 'bbb', usr: 'bbb' }, v: true }]
    }
  }
    ...
    .ready(function() {
      ...
      aaa
        ...
        .save$(function(err, foo, meta) {
          ...
          aaa.make$('foo').load$(1, function(err, foo2) {
            expect(foo2.mark).equal('a')
            expect(foo2.id).equal(foo.id)

            bbb.make$('foo').load$(1, function(err, foo) {
  ...
  Seneca()
    ...
    .ready(function() {
      ...
      this.act(
        ...
        function(err, out) {
          expect(out).equal({
            perms: [{ p: { usr$: 'aaa', usr: 'aaa' }, v: true }],
            usr$: 'aaa'
          })
          ...
          // as null == perm param
          this.act('role:allow,upon:perm,op:add,tgrp:nopermsatall', function(
            err,
            out
          ) {
...
lab.test('org-admin', fin => {
  ...
  Seneca()
    ...
    .use(function() {
      this.add('role:entity,cmd:save', function(msg, reply) {
        msg.ent.usr = msg.usr
        msg.ent.org = msg.org
    ...
    .ready(function() {
      var ccc = ud(this, 'ccc', 'QQQ')
      var ddd = ud(this, 'ddd', 'QQQ')
      ...
      ccc
        ...
        .save$(function(err, bar) {
          ...
          ccc.make$('bar').load$(1, function(err, bar2) {
            expect(bar2.mark).equal('b')
            expect(bar2.id).equal(bar.id)

            ddd.make$('bar').load$(1, function(err, bar3) {
...
const permspecs_alice_bob_org0 = {
  alice: {
    usr$: 'alice',
    perms: [{ p: { usr$: 'alice', usr: 'alice' }, v: true }]
  },
  ...
  read0: {
    grp: 'read0',
    perms: [{ p: { ent$: true, cmd$: 'load' }, v: true }]
  },
  ...
  Seneca()
    ...
    .ready(function() {
      ...
      exp.resolve_perms(this, { usr: 'alice' }, function(ignore, out) {
        ...
        exp.resolve_perms(this, { usr: 'alice', org: 'org0' }, function(
          ignore,
          out
        ) {
          //console.log(out)
          ...
          exp.resolve_perms(this, { usr: 'bob' }, function(ignore, out) {
            ...
            exp.resolve_perms(this, { usr: 'bob', org: 'org0' }, function(
              ignore,
              out
            ) {
              //console.log(out)
...
lab.test('access-org-basic', fin => {
  Seneca()
    ...
    .ready(function() {
      var alice_org0 = ud(this, 'alice', 'org0')
      var bob_org0 = ud(this, 'bob', 'org0')

      // alice can write
      alice_org0.make$('zed', { id$: 1, mark: 'a' }).save$(function(err, zed) {
        if (err) return fin(err)
        expect(zed).exist()

        // bob can't write
        bob_org0.make$('zed', { id$: 2, mark: 'b' }).save$(function(err, zed) {
          //console.log(err,zed)
          expect(err.code).equal('no_write_access')
          expect(zed).not.exist()

          // alice can read
          alice_org0.make$('zed').load$({ id: 1 }, function(err, zed) {
            //console.log(zed)
            if (err) return fin(err)
            expect(zed).exist()

            // bob can read
            bob_org0.make$('zed').load$({ id: 1 }, function(err, zed) {
              //console.log(zed)
              if (err) return fin(err)
              expect(zed).exist()
...
lab.test('access-org-field', fin => {
  const permspecs_alice_bob_org1 = {
    ...
    read1: {
      grp: 'read1',
      perms: [{ p: { ent$: true, cmd$: 'load', mark: 'a' }, v: true }]
    },
  ...
  Seneca()
    ...
    .ready(function() {
      var alice_org1 = ud(this, 'alice', 'org1')
      var bob_org1 = ud(this, 'bob', 'org1')

      // alice can write
      alice_org1.make$('zed', { id$: 1, mark: 'a' }).save$(function(err, zed) {
        if (err) return fin(err)
        expect(zed).exist()
      ...
      function do_access(alice_org1, bob_org1) {
        // bob can't write
        bob_org1.make$('zed', { id$: 2, mark: 'a' }).save$(function(err, zed) {
          //console.log(err,zed)
          expect(err.code).equal('no_write_access')
          expect(zed).not.exist()

          // alice can read
          alice_org1.make$('zed').load$({ id: 1 }, function(err, zed) {
            //console.log(zed)
            if (err) return fin(err)
            expect(zed).exist()

            // bob can't read mark:b
            bob_org1.make$('zed').load$({ id: 2 }, function(err, zed) {
              //console.log(zed)
              expect(err.code).equal('no_read_access')
              expect(zed).not.exist()

              // bob can read mark:a
              bob_org1.make$('zed').load$({ id: 1 }, function(err, zed) {
                //console.log(zed)
...
lab.test('access-org-readwrite', fin => {
  const permspecs_alice_bob_org2 = {
    ...
    cathy: {
      usr$: 'cathy',
      perms: [{ p: { usr$: 'catch', usr: 'cathy' }, v: true }]
    },
  ...
  Seneca()
    ...
    .ready(function() {
      var alice_org2 = ud(this, 'alice', 'org2')
      var bob_org2 = ud(this, 'bob', 'org2')
      var cathy_org2 = ud(this, 'cathy', 'org2')

      // alice can write a
      alice_org2.make$('qaz', { id$: 1, mark: 'a' }).save$(function(err, qaz1) {
        if (err) return fin(err)
        expect(qaz1).exist()
      ...
      function do_access(alice_org2, bob_org2, cathy_org2) {
        // bob can't write
        bob_org2.make$('qaz', { id$: 4, mark: 'a' }).save$(function(err, qaz4) {
          //console.log(err,qaz)
          expect(err.code).equal('no_write_access')
          expect(qaz4).not.exist()

          // alice can read
          alice_org2.make$('qaz').load$({ id: 1 }, function(err, qaz) {
            //console.log(qaz)
            if (err) return fin(err)
            expect(qaz).exist()

            // bob can't read mark:b
            bob_org2.make$('qaz').load$({ id: 2 }, function(err, qaz2) {
              //console.log(qaz)
              expect(err.code).equal('no_read_access')
              expect(qaz2).not.exist()

              // bob can read mark:a
              bob_org2.make$('qaz').load$({ id: 3 }, function(err, qaz3) {
                //console.log(qaz)
                expect(err).not.exist()
                expect(qaz3).exist()
      ...
      function do_list(alice_org2, bob_org2, cathy_org2) {
        // alice lists 1,3
        alice_org2.make$('qaz').list$({}, function(err, list0) {
          expect(err).not.exist()
          ...
          // bob lists 1,3
          bob_org2.make$('qaz').list$({}, function(err, list1) {
            expect(err).not.exist()
            ...
            // cathy lists 1,2,3
            cathy_org2.make$('qaz').list$({}, function(err, list2) {
              expect(err).not.exist()
...
lab.test('access-msg', fin => {
  const permspecs_org3 = {
    ...
    canfoo: {
      grp: 'canfoo',
      perms: [{ p: { role: 'bar', cmd: 'foo' }, v: true }]
    },
  ...
  Seneca()
    ...
    .ready(function() {
      var alice_org3 = ud(this, 'alice', 'org3')
      var bob_org3 = ud(this, 'bob', 'org3')
...
lab.test('intern-get_perms', fin => {
  ...
  Plugin.intern.get_perms(store, { usr: 'alice' }, function(err, out) {
    ...
    Plugin.intern.get_perms(store, { usr: 'bob', org: 'aaa' }, function(
      err,
      out
    ) {
      if (err) return fin(err)
...
lab.test('intern-make_grps_key', fin => {
  ...
  expect(function() {
    Plugin.intern.make_grps_key({})
  }).throws()
  expect(function() {
    Plugin.intern.make_grps_key({ usr: 'a' })
  }).throws()
  expect(function() {
    Plugin.intern.make_grps_key({ org: 'b' })
  }).throws()
  expect(function() {
    Plugin.intern.make_grps_key({ usr: '', org: 'b' })
  }).throws()
  expect(function() {
    Plugin.intern.make_grps_key({ usr: 'a', org: '' })
  }).throws()
  expect(function() {
    Plugin.intern.make_grps_key({ grp: 'c' })
  }).throws()
  fin()
})
...
lab.test('intern-grp', fin => {
  ...
  Plugin.intern.get_grps(store, { usr: 'alice', org: 'reds' }, function(
    err,
    out
  ) {
    if (err) return fin(err)
    ...
    Plugin.intern.grp_update(
      store,
      { op: 'add', tusr: 'alice', torg: 'reds', tgrp: 'g1' },
      function(err, out) {
        if (err) return fin(err)
        ...
        Plugin.intern.grp_update(
          store,
          { op: 'rem', tusr: 'alice', torg: 'reds', tgrp: 'g0' },
          function(err, out) {
            if (err) return fin(err)
...
lab.test('intern-perm', fin => {
  ...
  Plugin.intern.perm_update(
    ...
    function(err, out) {
      ...
      //console.dir(out,{depth:null})
      expect(out).equal({
        perms: [{ p: { ent$: true, usr$: 'alice', usr: 'alice' }, v: true }],
        usr$: 'alice'
      })
      ...
      Plugin.intern.perm_update(
        ...
        function(err, out) {
          ...
          expect(out).equal({
            perms: [
              { p: { ent$: true, usr$: 'alice', usr: 'alice' }, v: true }
            ],
            usr$: 'alice'
          })
...
lab.test('perm-access', fin => {
  ...
  const permspecs = {
    ...
    'owner-bar-a': {
      ...
      perms: [
        ...
        {
          p: { ent$: true, cmd$: '*', org$: 'greens', name$: 'bar', mark: 'a' },
          v: true
        }
      ]
    },
    ...
    write: {
      grp$: 'write',
      org$: 'greens',
      perms: [{ p: { ent$: true, cmd$: '*', org$: 'greens' }, v: true }]
    },
  ...
  Seneca()
    ...
    .ready(function() {
      var alice = ud(this, 'alice', 'greens')
      var bob = ud(this, 'bob', 'greens')
      var cathy = ud(this, 'cathy', 'greens')
      var derek = ud(this, 'derek', 'greens')
      var eoin = ud(this, 'eoin', 'greens')
      var fred = ud(this, 'fred', 'greens')
                        ...
                        function(err, out) {
                          ...
                          bob.act(
                            ...
                            function(err, out) {
                              ...
                              expect(d(this)['fred~greens'].grps).equal([
                                'read'
                              ])
                ...
                function(err, out) {
                  ...
                  derek.act(
                    ...
                    function(err, out) {
                      ...
                      eoin
                        .make$('bar', { id$: 1, mark: 'a' })
                        .save$(function(err, out) {
                          expect(out.mark).equal('a')

                          eoin
                            .make$('bar', { id$: 2, mark: 'b' })
                            .save$(function(err, out) {
                              expect(err.code).equal('no_write_access')
...
function ud(instance, usr, org) {
  return instance.delegate(null, { custom: { allow: { usr: usr, org: org } } })
}
