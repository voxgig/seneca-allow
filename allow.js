...
module.exports = function allow(options) {
  ...
  function get_perms(msg, reply, meta) {
    return intern.get_perms(store, msg, reply, meta)
  }

  function get_grps(msg, reply, meta) {
    return intern.get_grps(store, msg, reply, meta)
  }
  ...
  function allow_entity(msg, reply, meta) {
    var principal = meta.custom.allow || {}
    if (!principal.usr) return reply(error('no_user', { msg: msg }))
    ...
    resolve_perms(this, principal, function(err, perms) {
      ...
      if ('save' === msg.cmd || 'remove' === msg.cmd) {
        activity = intern.make_entity_activity(principal, msg, msg.ent)
      ...
      // case: read operation, ent returned
        ...
        return this.prior(msg, function(err, out, meta) {
          ...
          activity = intern.make_entity_activity(principal, msg, out)

          const access = perms.find(activity)

          if (!access) {
            // TODO: fix - data should not be sent back
  ...
  function resolve_perms(seneca, principal, done) {
    const key = intern.make_perms_key(principal)
    var found
    ...
    seneca.act('role:allow,get:perms', principal, function(err, permspec) {
      if (err) return done.call(this, err)

      //found = perms_cache[key] = intern.make_perms(principal, permspec)
      found = intern.make_perms(principal, permspec)
...
const intern = (module.exports.intern = {
  make_perms_key: function(principal) {
    const usr = principal.usr
    const org = principal.org
    const grp = principal.grp
    ...
    if ('' === key) {
      throw error('no_key_in_principal', { principal: principal })
    }

    return key
  },

  make_grps_key: function(principal) {
    var usr = principal.usr
    ...
    if (null == usr || null == org) {
      throw error('non_empty_usr_org_required', { principal: principal })
    }

    return usr + '~' + org
  },

  get_grps: function(store, msg, reply, meta) {
    var principal = (meta && meta.custom.allow) || {}

    // Use meta principal if not defined explicitly
    const usr = msg.usr || principal.usr
    const org = msg.org || principal.org

    const key = intern.make_grps_key({ usr: usr, org: org })
    store.get(key, function(err, out) {
      if (err) return reply(err)
      const grps = out && out.grps
      reply(null, { usr: usr, org: org, grps: grps })
    })
  },

  get_perms: function(store, msg, reply, meta) {
    var principal = (meta && meta.custom.allow) || {}
    ...
    function addperm(what, principal) {
      waiting++
        ...
        if (out && out.perms) {
          if (principal) {
            out.perms.forEach(function(perm) {
              Object.keys(principal).forEach(function(field) {
                perm.p[field] = principal[field]
              })
            })
          }
  ...
  perm_update: function(store, msg, reply) {
    ...
    if ('add' === op) {
      store.sadd(key, 'perms', perm, annot, function(err, out) {
        if (err) return reply(err)
        reply(null, out)
      })
    } else if ('rem' === op) {
      store.srem(key, 'perms', perm, annot, function(err, out) {
        if (err) return reply(err)
        reply(null, out)
      })
    } else {
  ...
  make_perms: function(principal, permspec) {
  ...
  make_entity_activity: function(principal, msg, ent) {
